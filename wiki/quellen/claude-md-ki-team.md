...
**Nachtrag 2026-08-01 (Feldinspektion statt Rückspielung):** Bis hierher entstand jede Aktualisierung aus dem, was der Architekt des Feldprojekts *berichtet* hat. Am 2026-08-01 wurde das Repo `website-maxron-de` erstmals **direkt gelesen** (read-only) und mit der Vorlage abgeglichen. Ergebnis: sechs Lücken, die kein Rückspielbericht gemeldet hatte, weil sie im Feldprojekt über 22 Kaskaden **nebenbei** entstanden sind und dort niemandem mehr auffielen. **(1) `## Projekt-Spezifika` fehlte ganz** — die reale `CLAUDE.md` beginnt damit (Tech-Stack, Produktivcode-Root, Smoke-Test, Deploy), und die Regel-Inventar-Einträge `R-1`…`R-8` stammen alle daher; besonders der **Smoke-Test-Befehl** ist konstitutiv, weil Ralphs Dreisatz ohne ihn nicht abschließbar ist. **(2) Bootstrap-Dateiliste fehlte** — welche Dateien vor dem ersten Lauf existieren müssen (CHANGELOG mit leerem `[Unreleased]`, Beutebuch **mit Vorlage-Block**, Roadmap, Backlog, Ermittlungsakten-Ordner, Briefings, Ledger). **(3) Beutebuch-Fundformat** war beschrieben, aber nicht als Block gezeigt. **(4) `.gitignore`-Liste** war unvollständig (es fehlten `.ralph-plan`, `.frank-attempts`, `.budget-ledger.lock` — die ersten beiden tauchen sonst im Read-Only-Guard als Fremdänderung auf). **(5) Briefing-Helfer und Briefing-Aufbau** waren als Prinzip dokumentiert, ohne Code und ohne Muster; der **Pflicht-Fallback** bei fehlender Datei ist dabei sicherheitsrelevant. **(6) Abschluss-Protokoll** hatte eine Inhaltsliste, aber keine Gliederung — die reale hat sieben Abschnitte, darunter „Bewertung des Bauwegs" und „Closeout-Funde", aus denen im Feld die beiden wertvollsten Lehren stammen (Prosa-Kosten und `BL-55`). Dazu die Test-Namenskonvention `test_<fundnummer>_<stichwort>.py`, die ~30 Regressionstests ohne Extra-Liste ihren Funden zuordnet.

> **Methodische Lehre**: Rückspielung durch Bericht findet, was der Berichtende für bemerkenswert hält. Sie findet **nicht**, was zur Selbstverständlichkeit geworden ist. Für eine Vorlage, die fremde Projekte aufsetzen soll, ist genau das die gefährlichere Lücke — der Erstlauf scheitert nicht an den spannenden Regeln, sondern an einer fehlenden leeren Datei. Direkte Inspektion des Feldprojekts sollte daher **zusätzlich** zur Rückspielung stattfinden, nicht statt ihrer.
