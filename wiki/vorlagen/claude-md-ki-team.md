...
**Modell / Plattform**: {{z.B. claude-sonnet-4-6 via Claude Code — Loop-Rollen; {{starkes-modell}} via API — Axel/Architekt}}
**Budget-Limit**: {{N}} USD pro Stufe (Ralph) | {{M}} USD pro Fall (Axel) | {{K}} USD gesamt
**Warnschwelle**: {{N×0,8}} USD pro Stufe/Fall

Kosten werden in `wiki/kosten.md` protokolliert (Abschluss-Variante — Projekt hat definiertes Ende pro Kaskade).

**Token-Sparregeln (stets aktiv):**
- Dateien nur einmal pro Stufe/Fall lesen
- `offset` + `limit` bei Dateien > 100 Zeilen (max. 50 Zeilen pro Abruf)
- Pro Antwort max. 20 Zeilen, wenn nicht mehr gefragt
- Bei Erreichen der Warnschwelle: {{Strippenzieher}} informieren, bevor weitergemacht wird

**Projektabschluss-Pflicht:**
Vor dem letzten Commit `wiki/kosten.md` mit vollständiger Gesamtauswertung abschließen — Tabelle aller Stufen/Fälle mit Kosten, Gesamtsumme, Kosteneffizienz-Notiz (z. B. Kosten pro Stufe / Kosten pro gefixtem Finding).
